...
- An active track tags newly imported backlog items, new items from `mano start`'s conversational intake, and follow-up review items. `mano start` considers only matching-track backlog items and records the track in its approved phase brief. A review uses that recorded phase track even if the local setting changes later.
