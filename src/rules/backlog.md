...
An item's `Status` says where it stands: `backlog` (open), `in-phase-N` / `in-owner-phase-N` (scoped into a phase), `resolved` (shipped or fixed), `rejected` (no longer wanted — its premise was invalidated). `resolved` and `rejected` are both closed states and neither is scopeable, but they are not interchangeable: recording a rejection as `resolved` claims work was done that never was. `mano review` sets `rejected` on human-confirmed items via `backlog.js reject --title "..."`. `mano import` may also use that writer when a document merge invalidates an existing `Status: backlog` item and the human has authorized rejecting the item or replacing its product direction (see `mano import` Step 1b). Import may append new items and merge context into existing `backlog` items, but never changes scoped or closed items. A merge request alone does not authorize rejection.

Apart from import’s document-merge boundary above, other skills should not edit the backlog except for narrow gap-resolution status updates:
