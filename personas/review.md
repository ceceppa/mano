...
1. Write ALL triaged items to `_mano_output/backlog.md` with categories preserved. Use the standard backlog item format (title, source, context, status). **Append only — never remove or replace existing items.**
