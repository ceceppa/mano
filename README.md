...
Review does not pretend that every completed phase was validated. It records what happened and keeps the exact phase checks visible. Where or how you checked the result is useful but optional. Mano omits that field when you do not supply it; it does not grade the review. `close it` is your sign-off, and Mano treats it as one: it records each exit criterion as signed off by you — in `progress.md` with the date, on the build path — and accepts the assumptions you shipped on. What it does not do is answer anything Mano asked you. Two slots are yours alone: what happened, and each Validation Question. If a close leaves either open, Mano comes back once — and only once — with a single line listing exactly those, because you did not write the brief's questions and have no reason to remember them at close time. This is where you find out what you were about to skip. `didn't check` and `didn't test it` are complete answers, recorded as yours, and whatever else you say in that reply is triaged like any other feedback. Mano never fills one in for you. Feedback is optional; an honest record of its absence is not. Empty worked/didn't-work sections, story counts, test counts, and shipped-feature summaries are omitted unless a fact directly supports the decision.
